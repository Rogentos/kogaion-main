...
SRC_URI="mirror://kogaion/${CATEGORY}/${MY_PN}/${MY_P}.tar.bz2
...
JABBER_DOC="${EPREFIX}/usr/share/doc/${MY_PF}"
...
src_prepare() {
	...
	# Set correct paths
	sed -e "/^EJABBERDDIR[[:space:]]*=/{s:ejabberd:${MY_PF}:}" \
		-e "/^ETCDIR[[:space:]]*=/{s:@sysconfdir@/ejabberd:${JABBER_ETC}:}" \
		-e "/^LOGDIR[[:space:]]*=/{s:@localstatedir@/log/ejabberd:${JABBER_LOG}:}" \
		-e "/^SPOOLDIR[[:space:]]*=/{s:@localstatedir@/lib/ejabberd:${JABBER_SPOOL}:}" \
			-i Makefile.in || die
	sed -e "/EJABBERDDIR=/{s:ejabberd:${MY_PF}:}" \
	...
	sed -e 's|\({captcha_cmd,[[:space:]]*"\).\+"}|\1/usr/'$(get_libdir)'/erlang/lib/'${MY_P}'/priv/bin/captcha.sh"}|' \
			-i ejabberd.cfg.example || die "Failed sed ejabberd.cfg.example"
...
src_configure() {
	econf \
		--docdir="${EPREFIX}/usr/share/doc/${MY_PF}/html" \
...
src_install() {
	...
	if use pam; then
		...
		fperms 4750 "/usr/$(get_libdir)/erlang/lib/${MY_PF}/priv/bin/epam" || die "Cannot adjust epam permissions"
	fi
	...
	newinitd "${FILESDIR}/${MY_PN}-3.initd" ${MY_PN} || die "Cannot install init.d script"
	newconfd "${FILESDIR}/${MY_PN}-3.confd" ${MY_PN} || die "Cannot install conf.d file"
}

pkg_postinst() {
	elog "For configuration instructions, please see"
	elog "/usr/share/doc/${MY_PF}/html/guide.html, or the online version at"
	...
	if [[ -e ${EROOT}/etc/jabber/ssl.pem ]]; then
		...
		ewarn "upgrading from ${CATEGORY}/${MY_PN}-2.0.5* or earlier  you might"
