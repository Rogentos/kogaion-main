...
SRC_URI="http://launchpad.net/elementaryicons/2.0/${PV}/+download/${P}.tar.gz
	branding? ( mirror://kogaion/x11-themes/fdo-icons-kogaion${SLREV}.tar.gz )"
...
src_install() {
	...
	if use branding ; then
		...
		for target in $(find "${D}"/usr/share/icons/*/panel/ -name start-here.svg); do
			cp fdo-icons-kogaion/scalable/places/start-here.svg "${target}" || die
		done
	fi
}
